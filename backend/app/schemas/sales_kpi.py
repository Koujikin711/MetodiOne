...
class SalesKpiCompanyReport(BaseModel):
    ...
    revenue_total: Decimal = Field(
        description="Clinic: visit cash only (OSV). Sales mode: desk + course KPI.",
    )
    revenue_booking: Decimal
    revenue_manual: Decimal = Field(
        description="Course/protocol KPI payments in month (shown separately; not in clinic revenue_total).",
    )
