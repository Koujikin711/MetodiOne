...
async def ensure_horeca_finance_tables(conn: AsyncConnection, database_url: str) -> None:
    """HoReCa: меню и техкарты для ресторанных расчётов (food-cost / COGS)."""
    low = database_url.lower()
    if "sqlite" in low:
        await conn.execute(
            text(
        ...
        await conn.execute(
            text(
                """CREATE TABLE IF NOT EXISTS horeca_tech_card_lines (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        await conn.execute(
            text(
                """CREATE TABLE IF NOT EXISTS horeca_menu_items (
                    ...
                    CONSTRAINT uq_horeca_menu_item_company_name UNIQUE (company_id, name)
                )""",
            ),
        )
        await conn.execute(
